...
class SignIn extends Component {
  ...
  render() {
    ...
    return (
      <View
        style={{ flex: 1, backgroundColor: "#e2e3e2", alignItems: "center" }}
      >
        <Text> {loginStatus} </Text>
        {this.state.currentUserId !== undefined ? logoutButton : loginButton}
        <Text marginTop={200} style={{ fontSize: 40, color: "black" }}>
          Expense Expert
        </Text>
        <TextInput
  ...
  _loadClient() {
    Stitch.initializeDefaultAppClient("expense-yqnnb").then(client => {
...
class DetailsScreen extends React.Component {
  ...
  render() {
    const width = 450;
    const height = 450;
    const sectionAngles = d3.pie().value(d => d.price)(
      ...
      >
        ...
        >
          {this.state.userPurchases.map((expense, key) => (
            <Text
              style={{ color: this.state.color[key], paddingLeft: 5 }}
              key={key}
            >
    ...
    return (
      ...
      >
        <Text style={{ paddingBottom: 10 }}>
          Enter a Category name, and press the Add Category button to create a
          new Category
        </Text>
        <TextInput
          style={styles.TextInputStyle}
        ...
        <Text style={{ paddingTop: 20 }}>
          Select a Category from the dropdown, and enter an expense value to add
          an Expense
        </Text>
        <Picker
...
class Remove extends React.Component {
  ...
  render() {
    return (
      ...
      >
        <Text>Select a Category to edit</Text>
        <Picker
          selectedValue={this.state.expense}
